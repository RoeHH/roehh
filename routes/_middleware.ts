...
import { gitHubApi, oauth2Client, User } from "../utils/oauth2.ts";
...
export async function handler(
  ...
  if (
    allowedProtocols.some((protocol) =>
      pathname.substring(1).startsWith(protocol)
    )
  ) {
    const short = await kv.get(["shorts", pathname.substring(1)], {
      consistency: "eventual",
    });
    if (req.method === "GET" && short.value) {
      return Response.redirect(short.value, 302);
    }
    if (pathname.substring(1).startsWith("s")) {
